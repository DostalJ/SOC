        ...
        try:
            ...
            for page in pages:
                if str(status.user.id) in followed[page]:
                    with open('./Data/sentiment/trump-{}.csv'.format(page), 'a') as out_file:
                        out_file.write(str(datetime.now()) + ',' + str(sent) + '\n')
                    with open('./Data/sentiment/trump-{}.txt'.format(page), 'a') as out_file:
...
while True:
    try:
        ...
        _log(e=e, log='./Data/sentiment/trump.log')
        pass
    if stop:
        print('Disconnecting.')
        myStream.disconnect()
